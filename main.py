...
class RocomPlugin(Star):
    ...
    def _get_current_announcement_interval_seconds(self) -> int:
        ...
        基于全网真实 947 篇历史动态与公告（B站 618 篇 + WeGame 329 篇）统计规律：
        1. 高频突发期（2 分钟 / 120 秒）：
           - 周三 18:00 ~ 21:30（版本大更新预告发帖核心区，单时段发帖 87 篇，全网第一峰）
           - 周四 09:30 ~ 12:30（更新日实机内容集中爆料区）
           - 周四 18:30 ~ 20:30（更新日晚间活动补充区）
           - 每日 09:50 ~ 11:30（全周每日 10:00 档期定点爆料窗口，全天候占 16.9% 断层第一峰）
        2. 低谷休眠期（30 分钟 / 1800 秒）：
           - 每日 23:00 ~ 次日 08:00（历史发帖率 < 1%，01:00~07:59 为绝对静默期，08:00 准时苏醒）
           - 每日 13:00 ~ 14:00（午休静默期，历史发帖率仅 0.74%）
        3. 常规活跃期：
           - 其余白天时段（如 08:00~09:50、11:30~13:00、14:00~18:00 等）使用配置的基础间隔（默认 10 分钟 / 600 秒）
        ...
        # 2. 低谷休眠期（30 分钟）
        # 每日 23:00 ~ 次日 08:00（微调：早 8 点准时唤醒切入常规活跃期）
        is_night_idle = (time_decimal >= 23.0 or time_decimal < 8.0)
        # 每日 13:00 ~ 14:00（午休静默）
    ...
    async def _run_merchant_ending_reminder(self):
        ...
        for key, sub in all_subs.items():
            ...
            if not matched:
                continue  # 本轮开盘未命中/数据不完整，无需结束提醒

            # 根据模式决定要提醒的商品
            if sub.get("ending_reminder_all_items", False):
                # 全部命中商品模式
