...
✅ **数据查询** - 个人档案可视化渲染、家园菜园/守卫/室内精灵、家园精灵完整数据、活动日历、公告查询与推送、近期对战详情、背包精灵图鉴检索、交换大厅和远行商人

✅ **收集排行** - 查询异色或炫彩精灵收集排行榜，可按 UID 附带指定玩家的当前名次和收集统计
...
✅ **阵容工具** - 登录后查看热门阵容推荐；无需登录即可解析原始阵容分享码或含 `shareData` 的链接，并查询后端历史记录
...
| `merchant_ending_reminder_template` | string | `"远行商人第{round}轮还有{countdown}结束，别忘了购买：{items}"` | 远行商人快结束提醒模板；`{items}` 替换为商品名，`{countdown}` 替换为剩余时间，`{round}` 替换为当前轮次 |
...
│   ├── client.py           # API 异步客户端
│   ├── user.py             # 用户数据中心
│   ├── ranking_service.py  # 精灵收集排行榜数据适配
│   ├── share_code_service.py # 阵容分享码提取与数据适配
│   └── render.py           # HTML 渲染助手
...
└── render/                 # 网页模板资源
    ...
    ├── pet-ranking/        # 异色/炫彩精灵收集排行榜模板
    ├── share-code-team/    # 阵容分享码可视化模板
...
| `异色排行榜 [UID] [数量]` | 查询异色精灵收集排行榜，数量支持 `1-50`；提供 UID 时附带该玩家当前名次（别名：`异色榜`、`洛克异色排行榜`） |
| `炫彩排行榜 [UID] [数量]` | 查询炫彩精灵收集排行榜，数量支持 `1-50`；提供 UID 时附带该玩家当前名次（别名：`炫彩榜`、`洛克炫彩排行榜`） |
...
| `订阅远行商人 [1/0] [商品...]` | 已开启对应权限的群管理员或 Bot 管理员可订阅远行商人提醒，`1` 为命中珍稀商品时提醒并 `@全体`，`0` 为普通提醒；`@` 前缀指定珍稀提醒商品；支持 `-N` / `-*N` 快结束提醒参数；不填商品则使用 WebUI 默认订阅商品 |
| `取消订阅远行商人` | 关闭当前群远行商人订阅 |
| `远行商人订阅状态` | 查看当前群/私聊远行商人的关注商品、强提醒模式及结束提醒配置（别名：`商人订阅状态`、`yxsr订阅状态`） |
...
| `阵容码 解析 <分享码/链接>` | 解析原始阵容分享码或含 `shareData` 的链接，展示精灵、血脉、性格、天赋和技能，无需登录 |
| `阵容码 查询 <分享码>` | 查询后端已记录的阵容分享码、阵容详情及解析次数，无需登录 |
...
| 指令 | 说明 |
| :--- | :--- |
| `洛克群发公告 [参数] <内容>` | 群发图文公告（默认生成沙盒预览并两步确认；可加 `--force` 或 `-y` 跳过确认；参数支持 `-d 延迟m`、`-t 时间`、`-a @全体`、`-g 全域`、`--active 活跃天数`、`--sub 订阅会话`） |
| `确认洛克群发 [确认码]` | 确认执行当前待确认的群发公告草稿并开始推送（60 秒内有效） |
| `取消洛克群发 [任务ID]` | 作废待确认草稿、紧急中止正在发送的任务或取消定时群发（别名：`终止洛克群发`） |
| `洛克群发任务列表` | 查看当前正在发送中、定时挂起中及待确认的任务与进度（别名：`洛克群发状态`、`群发任务`） |
| `暂停洛克群发 <任务ID>` | 暂停指定的定时群发任务，暂停期间到期不会触发发送 |
| `恢复洛克群发 <任务ID>` | 恢复已暂停的定时群发任务，将在设定时间正常执行 |
...
#### 新增
- 新增 `/异色排行榜 [UID] [数量]` 与 `/炫彩排行榜 [UID] [数量]`，接入玩家名片精灵收集排行榜接口，支持展示前 `1-50` 名和指定 UID 的当前名次。
- 新增 `/阵容码 解析 <分享码/链接>`，支持解析原始分享码或从含 `shareData` 的链接提取分享码，无需登录即可查看精灵、血脉、性格、天赋和技能。
- 新增 `/阵容码 查询 <分享码>`，查询后端已记录的分享码详情、解析次数和最近记录时间。

#### 优化
- 排行榜和阵容码使用独立轻量化模板，远程头像、精灵图或技能图加载失败时使用本地资源兜底，并保留文字结果回退。
- 阵容码适配后端记录的嵌套数据结构和相对资源地址；魔法缺少名称时使用魔法 ID 生成可读名称。
